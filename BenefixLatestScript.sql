...
GO
/****** Object:  StoredProcedure [dbo].[GetBenefexMonthlyData]    Script Date: 12/11/2019 11:09:42 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[GetBenefexMonthlyData]
  @taxPeriod int,
  @taxYear  int 
AS
/*
...
-- 3 -- Amend Captured Data:
 ...
 -- Set OTE Value:
  ...
  -- Change Non Sal for Zero Hour Contract
		UPDATE #LatestData SET  [Non Sal Sacr]  = 'Y' 
        FROM #LatestData 
        WHERE  [ContractType (Snapshot Status)] = 'Zero Hours contract' 


 -- Change Non Sal for Temporary Contract Less than 13 mnths

	    UPDATE #LatestData SET  [Non Sal Sacr]  = 'Y' 
        FROM #LatestData 
        WHERE  [ContractType (Snapshot Status)] = 'Temporary contract' 
				and  CONVERT(DATETIME,[Start date (Start Date)],103) >=  CONVERT(VARCHAR(20),dateadd(month,-13,getdate()),103)



  -- 1.	[FTEHours (Empl. Career Hist. FTE)] 
		UPDATE #LatestData SET  [FTEHours (Empl. Career Hist. FTE)] = 
		                        CASE WHEN [ContractType (Snapshot Status)] = 'Zero Hours contract' THEN 0 
								ELSE 1
								END 
        FROM #LatestData 
       

  -- 1.	2.	[Daysperweek (Days Per Week)] 
		UPDATE #LatestData SET  [Daysperweek (Days Per Week)]  = 
		                        CASE WHEN [ContractType (Snapshot Status)] = 'Zero Hours contract' THEN 0 
								ELSE 1
								END 
        FROM #LatestData 
